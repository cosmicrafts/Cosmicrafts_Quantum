...
using MetaDatum = System.ValueTuple<System.String, CanisterPK.testicrc1.Models.Value>;
using Balance1 = EdjCase.ICP.Candid.Models.UnboundedUInt;

namespace CanisterPK.testicrc1
{
	public class Testicrc1ApiClient
	{
		...
		public Testicrc1ApiClient(IAgent agent, Principal canisterId, CandidConverter? converter = default)
		{
			this.Agent = agent;
		...
		public async Task<Testicrc1ApiClient.Icrc1MetadataReturnArg0> Icrc1Metadata()
		{
			CandidArg arg = CandidArg.FromCandid();
			QueryResponse response = await this.Agent.QueryAsync(this.CanisterId, "icrc1_metadata", arg);
			CandidArg reply = response.ThrowOrGetReply();
			return reply.ToObjects<Testicrc1ApiClient.Icrc1MetadataReturnArg0>(this.Converter);
		}
